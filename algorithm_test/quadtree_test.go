...
type QuadNode algorithm.QuadNode

func TestQuadtree(t *testing.T){
	rand.Seed(time.Now().UnixNano())
	const width = 10000000
	const height = 10000000
	tree := algorithm.MakeQuadTree(0.0,0.0,algorithm.Float(width),algorithm.Float(height))
	const count =5000
	...
	for j:=0 ; j<threadCount; j++ {
		go func() {
			for i := 0; i < count; i++ {
				tree.Add(algorithm.Float(rand.Int31n(width*100))/100.0, algorithm.Float((rand.Int31n(height*100))/100.0))
			}
			wg.Done()
		}()
	}
